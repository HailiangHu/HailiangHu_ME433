...
int main(void) {
    ...
    sprintf(message,"%u",input);
    //LCD_drawMessage( x, y, message,GREEN,CYAN);
    
    
    while(1)
    {
    //while( _CP0_GET_COUNT() <= (SYS_FREQ/1000/2)*500 )// count up to Xms // greater than (SYS_FREQ /2*countingseconds)
    {;}
    //_CP0_SET_COUNT(0);
